...
function main() {
    ...
    const accounts = JSON.parse(readFileSync(manifestPath, 'utf8'));
    ...
    for (const a of accounts) {
        try {
            ...
            scorer.backfill(a.userId, events);
            const risk = scorer.getRisk(a.userId);
            injected++;
            console.log(`  ${risk?.level ?? '?'} ${String(risk?.score ?? '?').padStart(3)}  ${a.email.padEnd(48)} ` +
...
main();
//# sourceMappingURL=backfill-risk.js.map