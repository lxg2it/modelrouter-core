...
function formatDiff(ours, theirs) {
...
async function main() {
    ...
    const ok = [];
    const drifted = [];
    const notInLiteLLM = []; // priceSource='litellm' but not found in JSON
    const manualModels = [];
    const notAnnotated = []; // priceSource missing — treat as manual, warn
    for (const m of allModels) {
        const key = `${m.provider}/${m.model}`;
        const source = m.priceSource ?? 'manual';
        if (source === 'manual') {
            if (m.priceSource === undefined)
                notAnnotated.push(key);
...
//# sourceMappingURL=check-prices.js.map