...
export function createAdminRouter(deps) {
    ...
    if (risk) {
        app.get('/risk', (c) => {
            ...
            const minLevel = (c.req.query('minLevel') ?? 'watch');
...
// ─── Stats queries ─────────────────────────────────────
function queryAdminStats(db, usageStore) {
    // ── Users ──
  ...
  `).all();
    // ── Requests ──
    const totalRequests = db.prepare(`SELECT COUNT(*) as n FROM usage_log`).get().n;
...
function svgBarChart(data, options = {}) {
    ...
    const vf = options.valueFormatter ?? ((v) => String(v));
    const W = 600;
    const H = 140;
