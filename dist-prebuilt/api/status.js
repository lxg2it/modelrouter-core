...
class StatusStore {
    ...
    `).all(`-${days} days`).map((r) => r.provider);
        ...
        ORDER BY day ASC
      `).all(provider, `-${days} days`);
            const rowMap = new Map(rows.map((r) => [r.day, r]));
            const dayStatuses = spine.map((date) => {
...
// ─── Router ───────────────────────────────────────────────
export function createStatusRouter(db) {
...
function statusColor(d) {
    if (d.successRate === null)
        return 'var(--surface2)'; // no data - gray
    if (d.successRate >= 0.99)
        return 'var(--green)';
    if (d.successRate >= 0.90)
        return 'var(--warn)';
    return 'var(--red)';
}
...
function renderStatusPage(overall, providers) {
