 ...
 */
export function isPolicyViolation(error, provider) {
 ...
 */
function maybeRecordViolation(deps, keyId, err, provider) {
...
async function handleNonStreaming(c, request, decision, deps, apiKey, startTime, satbillAccountId, user, blockedProviders, telemetrySpan, freeProvidersOnly) {
    ...
    try {
        ...
        if (decision.pinned && !decision.fallbackChain?.length) {
            ...
            if (strikeMsg)
                errorMessage = `${strikeMsg} ${errorMessage}`;
            deps.logger.log({
                keyId,
                provider: decision.provider,
                model: decision.model,
        ...
        if (exhaustedStrike)
            exhaustedMsg = `${exhaustedStrike} ${exhaustedMsg}`;
        return c.json({
            error: {
...
async function handleStreaming(c, request, decision, deps, apiKey, startTime, satbillAccountId, user, blockedProviders, telemetrySpan, freeProvidersOnly) {
    ...
    if (!completion) {
        ...
        if (decision.pinned) {
            ...
            if (streamStrikeMsg)
                streamErrMessage = `${streamStrikeMsg} ${streamErrMessage}`;
            telemetrySpan?.end({
