...
export interface AutoTierResult {
  ...
  signals: SignalBreakdown; // Per-signal scores for transparency
}
 ...
 */
export function classifyAutoTier(messages: ChatMessage[]): AutoTierResult {
  ...
  const weights: [keyof SignalBreakdown, number][] = [
 ...
 */
function scoreSystemPrompt(messages: ChatMessage[]): number {
 ...
 */
function scoreCodeBlocks(messages: ChatMessage[]): number {
  let fencedCount = 0;
  let inlineCount = 0;
  let codeLineCount = 0;
  ...
  for (const msg of messages) {
    ...
    if (fenced) fencedCount += fenced.length;
    const inline = text.match(INLINE_CODE_RE);
    if (inline) inlineCount += inline.length;
    if (CODE_LINE_RE.test(text)) codeLineCount++;
  }

  // Fenced blocks are the strongest signal
  if (fencedCount >= 5) return 100;
  if (fencedCount >= 3) return 80;
  if (fencedCount >= 1) return 60;

  // Inline code is moderate
  if (inlineCount >= 10) return 50;
  if (inlineCount >= 3) return 30;

  // Code-like lines are weak signal
  if (codeLineCount >= 3) return 25;
  if (codeLineCount >= 1) return 15;
 ...
 */
function scoreTechnicalKeywords(messages: ChatMessage[]): number {
  ...
  for (const kw of PREMIUM_KEYWORDS) {
    if (fullText.includes(kw)) premiumHits++;
  }
  for (const kw of STANDARD_KEYWORDS) {
    if (fullText.includes(kw)) standardHits++;
  }

  // Premium keywords are strong signals
  if (premiumHits >= 3) return 100;
  if (premiumHits >= 2) return 80;
  if (premiumHits >= 1) return 60;

  // Standard keywords need more density to be conclusive
  if (standardHits >= 8) return 70;
  if (standardHits >= 5) return 50;
  if (standardHits >= 3) return 35;
  if (standardHits >= 1) return 20;
 ...
 */
function scoreConversationDepth(messages: ChatMessage[]): number {
  // Count user + assistant turns (exclude system)
  const turns = messages.filter((m) => m.role === 'user' || m.role === 'assistant').length;

  if (turns <= 1) return 0;
  if (turns <= 2) return 15;
  if (turns <= 4) return 30;
  if (turns <= 8) return 50;
 ...
 */
function scoreToolUsage(messages: ChatMessage[]): number {
  let toolCalls = 0;
  let toolResults = 0;

  for (const msg of messages) {
    if (msg.tool_calls && msg.tool_calls.length > 0) toolCalls += msg.tool_calls.length;
 ...
 */
function scoreMessageComplexity(messages: ChatMessage[]): number {
  ...
  if (maxLength < 50) return 5;
  if (maxLength < 200) return 20;
  if (maxLength < 500) return 35;
  if (maxLength < 1500) return 55;
  if (maxLength < 5000) return 75;
  return 90;
}
 ...
 */
function scoreReasoningMarkers(messages: ChatMessage[]): number {
  const fullText = messages.map(extractText).join(' ').toLowerCase();

  let hits = 0;
  for (const marker of REASONING_MARKERS) {
    if (fullText.includes(marker)) hits++;
  }
