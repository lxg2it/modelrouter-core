 ...
 */
function translateUserMessage(msg: AnthropicMessageParam): ChatMessage[] {
  ...
  for (const block of msg.content) {
    ...
    // image, document blocks in user messages → best effort text
      ...
      const img = block as { source: { media_type: string } };
 ...
 */
function translateAssistantMessage(msg: AnthropicMessageParam): ChatMessage {
  if (typeof msg.content === 'string') {
...
function translateTools(tools: AnthropicToolParam[]): Tool[] {
  return tools.map((t) => ({
