...
function translateMessages(messages: ChatMessage[]): {
  ...
  for (const msg of messages) {
    ...
    if (msg.role === 'tool') {
      ...
      const resultContent: ToolResultContentBlock[] = [{ text: toolMsg.content ?? '' }];
      bedrockMessages.push({
        role: 'user',
 ...
 */
function buildContentBlocks(msg: ChatMessage): ContentBlock[] {
  if (msg.role === 'assistant') {
    ...
    if (aMsg.content) blocks.push({ text: aMsg.content });
    if (aMsg.tool_calls) {
      for (const tc of aMsg.tool_calls) {
        ...
        blocks.push({ toolUse: { toolUseId: tc.id, name: tc.function.name, input: inputObj as DocumentType } });
      }
    }
    return blocks;
  }
    ...
    for (const part of msg.content) {
      if (part.type === 'text' && part.text) {
        ...
        if (url.startsWith('data:')) {
          ...
          if (match) {
            const mimeType = match[1] as 'image/png' | 'image/jpeg' | 'image/gif' | 'image/webp';
 ...
 */
function translateTools(tools?: ChatCompletionRequest['tools']): { toolConfig?: { tools: Tool[] } } {
  ...
  const bedrockTools: Tool[] = tools.map((t) => ({
    toolSpec: {
      ...
      inputSchema: {
        json: (t.function.parameters as ToolInputSchema['json']) ?? {},
      } as ToolInputSchema,
    },
  }));
...
function isContextLengthError(err: unknown): boolean {
  if (!(err instanceof ValidationException)) return false;
...
export class BedrockAdapter implements ProviderAdapter {
  ...
  async complete(
    ...
    if (!this.client) throw new Error('Bedrock adapter not configured');
    ...
    try {
      ...
      const finishReason = (() => {
        switch (response.stopReason) {
          case 'end_turn': return 'stop' as const;
          case 'max_tokens': return 'length' as const;
          case 'tool_use': return 'tool_calls' as const;
          default: return 'stop' as const;
        }
      })();
  ...
  async stream(
    ...
    try {
      ...
      if (timer) clearTimeout(timer);
      if (isContextLengthError(err)) {
        throw new ContextLengthExceededError((err as Error).message);
      }
      throw err;
    }
