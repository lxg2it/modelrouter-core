...
export function createCompletionsRouter(deps: ChatDeps): Hono<AuthEnv> {
  ...
  app.post('/', async (c) => {
    ...
    const body = await c.req.json<TextCompletionRequest>();
    ...
    try {
      ...
      const costCents = foundModelConfig
        ? UsageLoggerClass.calculateCost(
            ...
            foundModelConfig.inputPer1M,
            foundModelConfig.outputPer1M,
          )
        : 0;
